...
class Solution:
    def cloneGraph(self, node: Optional[Node]) -> Optional[Node]:
        ...
        while q:
            ...
            for n in v.neighbors:
                if n.val not in output:
                    output[n.val] = Node(n.val)
                    q.append(n)
                curr.neighbors.append(output[n.val])
    ...
    def cloneGraph2(self, node: Optional[Node]) -> Optional[Node]:
        ...
        def clone(node):
            ...
            if node.val in output:
                return output[node.val]

            output[node.val] = Node(node.val)

            for n in node.neighbors:
                if n.val not in output[node.val].neighbors:
                    output[node.val].neighbors.append(clone(n))
            return output[node.val]
